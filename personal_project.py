...
min_salary_point = int(input('Введите точку отработки оклада, в процентах (в том случае, если результативность будет меньше данной отметки, сотрудник будет получать только оклад): '))
max_salary_point = int(input('Введите точку отработки премии, в процентах (премия за дополнительную результативность будет начисляться при результативности не более данного значения): '))
